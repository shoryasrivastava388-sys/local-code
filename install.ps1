# Windows PowerShell Installer for qwen-agent (qc)
...
$ScriptUrl = "https://raw.githubusercontent.com/shoryasrivastava388-sys/qwen-agent/main/qwen_agent.py"
$TargetPy = "$InstallDir\qwen_agent.py"

Write-Host "→ Downloading qwen-agent to $InstallDir..." -ForegroundColor Cyan
Invoke-WebRequest -Uri $ScriptUrl -OutFile $TargetPy -UseBasicParsing

# Create Windows .cmd wrappers so 'qc' and 'qwen-agent' run directly from CMD / PowerShell
...
Write-Host "✓ Successfully installed qwen-agent and qc to $InstallDir" -ForegroundColor Green
Write-Host ""
...
Write-Host ""
Write-Host "Usage:"
Write-Host "  qc                 # Interactive mode"
Write-Host "  qc -y 'Your task'  # Auto-approve mode"
